...
void Trevor::connect_user(const QString& user)
{
    ...
    if(n_users == 1){
        ...
        emit energyConsumption(1, double(time.elapsed())/1E3, compute_energy_consumption());
    }
...
double Trevor::compute_energy_consumption(){
    double T = double(time.elapsed())/1E3;

    energy_used += V * ((T+time_server_connect) - Ti)*I;

    Ti = T+time_server_connect;
...
void Trevor::subscribeToTopics()
{
    ...
    time_server_connect = double(time.elapsed())/1E3;
